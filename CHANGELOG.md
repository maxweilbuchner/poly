...
### Changed
- Markets list redesigned: single-line columned rows (Question · Prices · Vol · Ends · Cat) with a header row, mirroring the Positions tab. Watchlist `★` and held-position `●` (colored by P&L direction) now live in a 2-char left-side gutter; trailing `↑ Outcome @P¢` badge dropped. Question column tightens to the longest visible question so the Prices column slides left when no row needs the extra width; orphan `·` between Market and Prices replaced with plain spacing. Prices column splits into two aligned (label, %) sub-cells; label width auto-fits the longest visible outcome name (capped at 10 chars), so `Yes`/`No` lists tighten to 3 chars while Over/Under or team-spread rows widen to fit. Prices column collapsed to the leading outcome only — `<label>  <12-cell █▉▊▋▌▍▎▏ probability bar>  <pct>` — eliminates the redundant complementary-price column on binary markets and makes magnitude scannable at a glance (matches Polymarket / Manifold / Kalshi list UIs). Bar shows a dim `░` track for the empty portion so every row reads as a complete progress widget. Question column now caps at 60 chars so very long titles truncate instead of stretching every row. `market_category` broadened to tag LoL/esports match-prop markets ("Game 1:", "slay a dragon", "any player quadra kill", …), boxing fight cards, and additional politics keywords (Bernie Sanders, Maduro, NYT front-page, etc.). Already-closed markets are dropped from the list (awaiting resolution, not actionable). Empty Cat/Ends cells render `—` instead of leaving an orphan separator. Volume column tiered by magnitude (`≥$10K` text-bright, `≥$100` dim, lower very-dim) so big markets jump out. `Cat` header renamed to `Category`.
